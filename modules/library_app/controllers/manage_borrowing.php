...
/**
 * Description of Manage_Borrowing
 *
 * @author No-CMS Module Generator
 */
class Manage_Borrowing extends CMS_Priv_Strict_Controller {
	...
	public function index(){
        ...
        // set model
        $crud->set_model($this->cms_module_path().'/grocerycrud_borrowing_model');

        // adjust groceryCRUD's language to No-CMS's language
        $crud->set_language($this->cms_language());

        // table name
        $crud->set_table($this->cms_complete_table_name('borrowing'));

        // set subject
        $crud->set_subject('Borrowing');

        // displayed columns on list
        $crud->columns('id_member','borrow_date','return_date','books');
        // displayed columns on edit operation
        $crud->edit_fields('id_member','borrow_date','return_date','books');
        // displayed columns on add operation
        $crud->add_fields('id_member','borrow_date','return_date','books');

        // caption of each columns
        $crud->display_as('id_member','Member');
        $crud->display_as('borrow_date','Borrow Date');
        $crud->display_as('return_date','Return Date');
        $crud->display_as('books','Books');
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$crud->set_relation('id_member', $this->cms_complete_table_name('member'), 'name');
		...
		$crud->callback_column('books',array($this, 'callback_column_books'));
		$crud->callback_field('books',array($this, 'callback_field_books'));

        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // render
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/manage_borrowing_view', $output,
            $this->cms_complete_navigation_name('manage_borrowing'));
	...
	public function before_delete($primary_key){
		// delete corresponding borrowing_detail
		$this->db->delete($this->cms_complete_table_name('borrowing_detail'),
	...
	public function after_insert_or_update($post_array, $primary_key){
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//
		// SAVE CHANGES OF borrowing_detail
		//  * The borrowing_detail data in in json format.
		//  * It can be accessed via $_POST['md_real_field_books_col']
		//
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$data = json_decode($this->input->post('md_real_field_books_col'), TRUE);
		$insert_records = $data['insert'];
		$update_records = $data['update'];
		$delete_records = $data['delete'];
		$real_column_names = array('id', 'id_physical_book');
		...
		foreach($update_records as $update_record){
			...
			$data['id_borrowing'] = $primary_key;
			$this->db->update($this->cms_complete_table_name('borrowing_detail'),
		...
		foreach($insert_records as $insert_record){
			...
			$data['id_borrowing'] = $primary_key;
			$this->db->insert($this->cms_complete_table_name('borrowing_detail'), $data);
	...
	public function callback_field_books($value, $primary_key){
	    $module_path = $this->cms_module_path();
		$this->config->load('grocery_crud');
        $date_format = $this->config->item('grocery_crud_date_format');

		if(!isset($primary_key)) $primary_key = -1;
		$query = $this->db->select('id, id_physical_book')
			->from($this->cms_complete_table_name('borrowing_detail'))
			->where('id_borrowing', $primary_key)
			->get();
		$result = $query->result_array();

		// get options
		$options = array();
		$options['id_physical_book'] = array();
		$query = $this->db->select('id,code')
           ->from($this->cms_complete_table_name('physical_book'))
           ->get();
		foreach($query->result() as $row){
			$options['id_physical_book'][] = array('value' => $row->id, 'caption' => $row->code);
		}
		$data = array(
			'result' => $result,
			'options' => $options,
		...
		return $this->load->view($this->cms_module_path().'/field_borrowing_books',$data, TRUE);
	}

	// returned on view
	public function callback_column_books($value, $row){
	    $module_path = $this->cms_module_path();
		$query = $this->db->select('id, id_physical_book')
			->from($this->cms_complete_table_name('borrowing_detail'))
			->where('id_borrowing', $row->id)
			->get();
		$num_row = $query->num_rows();
		// show how many records
		if($num_row>1){
			return $num_row .' Borrowing Details';
		}else if($num_row>0){
			return $num_row .' Borrowing Detail';
		}else{
			return 'No Borrowing Detail';
		}
	}
