 ...
 */
class Manage_country extends CMS_Secure_Controller {
    ...
    private function make_crud(){
        ...
        /*
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('read_country'))){
            $crud->unset_read();
        }
        */

        // privilege to add
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('add_country'))){
            $crud->unset_add();
        }

        // privilege to edit
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('edit_country'))){
            $crud->unset_edit();
        }

        // privilege to delete
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('delete_country'))){
            $crud->unset_delete();
        }

        // privilege to list (uncomment if you need it)
        /*
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('list_country'))){
            $crud->unset_list();
        }
        */

        // privilege to back_to_list
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('back_to_list_country'))){
            $crud->unset_back_to_list();
        }

        // privilege to print
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('print_country'))){
            $crud->unset_print();
        }

        // privilege to export
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('export_country'))){
            $crud->unset_export();
        }
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('country'),array('country_id'=>$id));
