 ...
 */
class Manage_hobby extends CMS_Secure_Controller {
    ...
    private function make_crud(){
        ...
        /*
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('read_hobby'))){
            $crud->unset_read();
        }
        */

        // privilege to add
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('add_hobby'))){
            $crud->unset_add();
        }

        // privilege to edit
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('edit_hobby'))){
            $crud->unset_edit();
        }

        // privilege to delete
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('delete_hobby'))){
            $crud->unset_delete();
        }

        // privilege to list (uncomment if you need it)
        /*
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('list_hobby'))){
            $crud->unset_list();
        }
        */

        // privilege to back_to_list
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('back_to_list_hobby'))){
            $crud->unset_back_to_list();
        }

        // privilege to print
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('print_hobby'))){
            $crud->unset_print();
        }

        // privilege to export
        if(!$this->cms_have_privilege($this->cms_complete_navigation_name('export_hobby'))){
            $crud->unset_export();
        }
    ...
    public function index(){
        ...
        $this->view($this->cms_module_path().'/Manage_hobby_view', $output,
            $this->cms_complete_navigation_name('manage_hobby'), $config);
    }
