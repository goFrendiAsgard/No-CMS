...
class Browse_city extends CMS_Secure_Controller {
    ...
    protected function do_override_url_map($URL_MAP){
        ...
        $URL_MAP[$module_path.'/browse_city/config'] = $navigation_name;
    ...
    public function get_data($page = 0, $keyword = ''){
        ...
        $data = array(
            ...
            'backend_url'                  => site_url($module_path.'/manage_city/index'),
            'record_template'              => $this->cms_get_config('example_city_record_template', TRUE),
        );
    ...
    public function template_config(){
        ...
        if(!$this->cms_have_privilege($this->n('edit_city_record_template'))){
            redirect($module_path . '/browse_city');    
        }
        // save changes
        if($this->input->post('template') !== NULL){
            $this->cms_set_config('example_city_record_template', $this->input->post('template'));
            redirect($module_path . '/browse_city');    
        }else{
            $this->load->library('cms_asset');
            $data = array('value' => $this->cms_get_config('example_city_record_template', TRUE));
            $this->view($module_path.'/Browse_city_template_config_view',$data,
                $this->n('browse_city'));
        }
    }
