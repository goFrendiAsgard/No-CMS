...
class Install_Model extends CI_Model{
    ...
    protected function insert_all_data(){
        ...
        $sql_list[] = $this->insert_widget('section_top_fix', 'Top Fix Section', '', '', 1, 1, 2, 1, '{{ widget_name:top_navigation }}', NULL);
        $sql_list[] = $this->insert_widget('section_banner', 'Banner Section', '', '', 1, 1, 3, 1, '<div class="jumbotron hidden-xs hidden-sm" style="margin-top:10px;">'.PHP_EOL.'  <img src ="{{ site_logo }}" style="max-width:20%; float:left; margin-right:10px; margin-bottom:10px;" />'.PHP_EOL.'  <h1>{{ site_name }}</h1>'.PHP_EOL.'  <p>{{ site_slogan }}</p>'.PHP_EOL.'</div>', NULL);
        $sql_list[] = $this->insert_widget('section_left', 'Left Section', '', '', 1, 1, 4, 1, '', NULL);
        $sql_list[] = $this->insert_widget('section_right', 'Right Section', '', '', 1, 1, 5, 1, '{{ widget_slug:sidebar }}<hr />{{ widget_slug:advertisement }}', NULL);
        $sql_list[] = $this->insert_widget('section_bottom', 'Bottom Section', '', '', 1, 1, 6, 1, '{{ site_footer }}', NULL);
        $sql_list[] = $this->insert_widget('left_navigation', 'Left Navigation', '', 'main/widget_left_nav', 1, 1, 7, 0, NULL, NULL);
        $sql_list[] = $this->insert_widget('top_navigation', 'Top Navigation', '', 'main/widget_top_nav', 1, 1, 8, 0, NULL, NULL);
        $sql_list[] = $this->insert_widget('quicklink', 'Quicklinks', '', 'main/widget_quicklink', 1, 1, 9, 0, NULL, NULL);
        $sql_list[] = $this->insert_widget('login', 'Login', 'Visitor need to login for authentication', 'main/widget_login', 2, 1, 10, 0, '<form action="{{ site_url }}main/login" method="post" accept-charset="utf-8"><label>Identity</label><br><input type="text" name="identity" value=""><br><label>Password</label><br><input type="password" name="password" value=""><br><input type="submit" name="login" value="Log In"></form>', 'sidebar, user_widget');
        $sql_list[] = $this->insert_widget('logout', 'User Info', 'Logout', 'main/widget_logout', 3, 1, 11, 1, '{{ language:Welcome }} {{ user_name }}<br />'.PHP_EOL.'<a href="{{ site_url }}main/logout">{{ language:Logout }}</a><br />', 'sidebar, user_widget');
        $sql_list[] = $this->insert_widget('social_plugin', 'Share This Page !!', 'Addthis', 'main/widget_social_plugin', 1, 1, 12, 1, '<div class="addthis_sharing_toolbox"></div>'.PHP_EOL.'<!-- Go to www.addthis.com/dashboard to customize your tools -->'.PHP_EOL.'<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-4ee44922521f8e39"></script>', 'sidebar');
