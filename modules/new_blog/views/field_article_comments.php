...
<style type="text/css">
	...
	#md_table_comments .md_field_comments_col{
		width:auto!important;
		min-width:50px!important;
		max-width:150px!important;
	}
	#md_table_comments .datepicker-input{
		width:auto!important;
		min-width:50px!important;
		max-width:100px!important;
	}
	#md_table_comments .chzn-container,
	#md_table_comments .chzn-drop{
		width:auto!important;
...
<input id="md_field_comments_add" class="btn" type="button" value="Add Comment" />
<br />
...
<script type="text/javascript">
	...
	function add_table_row_comments(value){
		...
		if(typeof(value) != 'undefined' && value.hasOwnProperty('date')){
			field_value = php_date_to_js(value.date);
		}
		component += '<td>';
		component += '<input id="md_field_comments_col_date_'+RECORD_INDEX_comments+'" record_index="'+RECORD_INDEX_comments+'" class="md_field_comments_col datepicker-input" column_name="date" type="text" value="'+field_value+'"/>';
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////////
		var field_value = ''
		if(typeof(value) != 'undefined' && value.hasOwnProperty('author_user_id')){
			field_value = value.author_user_id;
		}
		component += '<td>';
		component += '<input id="md_field_comments_col_author_user_id_'+RECORD_INDEX_comments+'" record_index="'+RECORD_INDEX_comments+'" class="md_field_comments_col numeric" column_name="author_user_id" type="text" value="'+field_value+'"/>';
		component += '</td>';
		...
		component += '<td>';
		component += '<input id="md_field_comments_col_name_'+RECORD_INDEX_comments+'" record_index="'+RECORD_INDEX_comments+'" class="md_field_comments_col" column_name="name" type="text" value="'+field_value+'"/>';
		component += '</td>';
		...
		component += '<td>';
		component += '<input id="md_field_comments_col_email_'+RECORD_INDEX_comments+'" record_index="'+RECORD_INDEX_comments+'" class="md_field_comments_col" column_name="email" type="text" value="'+field_value+'"/>';
		component += '</td>';
		...
		if(typeof(value) != 'undefined' && value.hasOwnProperty('website')){
			field_value = value.website;
		}
		component += '<td>';
		component += '<input id="md_field_comments_col_website_'+RECORD_INDEX_comments+'" record_index="'+RECORD_INDEX_comments+'" class="md_field_comments_col" column_name="website" type="text" value="'+field_value+'"/>';
		component += '</td>';
		...
		component += '<td>';
		component += '<input id="md_field_comments_col_content_'+RECORD_INDEX_comments+'" record_index="'+RECORD_INDEX_comments+'" class="md_field_comments_col" column_name="content" type="text" value="'+field_value+'"/>';
		component += '</td>';
	...
	$(document).ready(function(){
        ...
        // md_field_comments_add.click (Add row)
        /////////////////////////////////////////////////////////////////////////////////////////////////////
		$('#md_field_comments_add').click(function(){
			// new data
			var data = new Object();
			...
			data.website = '';
			data.content = '';
			// insert data to the DATA_comments
			DATA_comments.insert.push({
				'record_index' : RECORD_INDEX_comments,
				'primary_key' : '',
				'data' : data,
			});

			// add table's row
			add_table_row_comments(data);
			// add  by 1
			RECORD_INDEX_comments++;

			// synchronize to the md_real_field_comments_col
			synchronize_comments();
		});
        ...
        // md_field_comments_col.change (Edit cell)
        /////////////////////////////////////////////////////////////////////////////////////////////////////
		$('.md_field_comments_col').live('change', function(){
			var value = $(this).val();
			...
			for(var i=0; i<DATA_comments.insert.length; i++){
				if(DATA_comments.insert[i].record_index == record_index){
					record_index_found = true;
					// insert value
					eval('DATA_comments.insert['+i+'].data.'+column_name+' = '+JSON.stringify(value)+';');
					break;
				}
			}
			if(!record_index_found){
				for(var i=0; i<DATA_comments.update.length; i++){
					if(DATA_comments.update[i].record_index == record_index){
						record_index_found = true;
						// edit value
						eval('DATA_comments.update['+i+'].data.'+column_name+' = '+JSON.stringify(value)+';');
						break;
					}
				}
			}
	...
	function mutate_input(){
		// datepikcer-input
		$('#md_table_comments .datepicker-input').datepicker({
		...
		$('#md_table_comments .datepicker-input-clear').click(function(){
			$(this).parent().find('.datepicker-input').val('');
			return false;
		});
		// chzn-select
		$("#md_table_comments .chzn-select").chosen({allow_single_deselect: true});
		// numeric
		$('#md_table_comments .numeric').numeric();
		$('#md_table_comments .numeric').keydown(function(e){
			if(e.keyCode == 38)
			{
