...
/**
 * Description of Manage_Order
 *
 * @author No-CMS Module Generator
 */
class Manage_Order extends CMS_Priv_Strict_Controller {
    ...
    public function index(){
        ...
        // set model
        // $crud->set_model($this->cms_module_path().'/grocerycrud_order_model');

        // adjust groceryCRUD's language to No-CMS's language
        $crud->set_language($this->cms_language());

        // table name
        $crud->set_table($this->cms_complete_table_name('order'));

        // set subject
        $crud->set_subject('Order');

        // displayed columns on list
        $crud->columns('name','address','phone_number_1','phone_humber_2','message','items');
        // displayed columns on edit operation
        $crud->edit_fields('name','address','phone_number_1','phone_humber_2','message','items');
        // displayed columns on add operation
        $crud->add_fields('name','address','phone_number_1','phone_humber_2','message','items');
        ...
        $crud->display_as('phone_humber_2','Phone Humber 2');
        $crud->display_as('message','Message');
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/manage_order_view', $output,
    ...
    public function before_delete($primary_key){
		// delete corresponding item_detail
		$this->db->delete($this->cms_complete_table_name('item_detail'),
    ...
    public function after_insert_or_update($post_array, $primary_key){
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//
		// SAVE CHANGES OF item_detail
		//  * The item_detail data in in json format.
		//  * It can be accessed via $_POST['md_real_field_items_col']
		//
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$data = json_decode($this->input->post('md_real_field_items_col'), TRUE);
		$insert_records = $data['insert'];
		$update_records = $data['update'];
		$delete_records = $data['delete'];
		$real_column_names = array('id', 'id_item', 'price', 'qty');
		...
		foreach($update_records as $update_record){
			...
			$data['id_order'] = $primary_key;
			$this->db->update($this->cms_complete_table_name('item_detail'),
		...
		foreach($insert_records as $insert_record){
			...
			$data['id_order'] = $primary_key;
			$this->db->insert($this->cms_complete_table_name('item_detail'), $data);
	...
	public function callback_field_items($value, $primary_key){
		...
		$query = $this->db->select('id, id_item, price, qty')
			->from($this->cms_complete_table_name('item_detail'))
			->where('id_order', $primary_key)
			->get();
		...
		return $this->load->view($this->cms_module_path().'/field_order_items',$data, TRUE);
	}
	...
	public function callback_column_items($value, $row){
		...
		if($num_row>1){
			return $num_row .' Item Details';
		}else if($num_row>0){
			return $num_row .' Item Detail';
		}else{
