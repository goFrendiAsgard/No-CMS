...
class Manage_project extends Nds_Special_CRUD_Controller {
    ...
    protected function make_crud(){
        ...
        $crud->display_as('db_server','Db Server');
        $crud->display_as('db_port','Db Port');
        $crud->display_as('db_schema','Db Schema');
        $crud->display_as('db_user','Db User');
        $crud->display_as('db_password','Db Password');
        $crud->display_as('db_table_prefix','Db Table Prefix');
    ...
    public function _callback_column_table($value, $row){
        return $this->_humanized_record_count(
                'table', // DETAIL TABLE NAME
                'project_id', // DETAIL FK NAME
                $row->project_id, // CURRENT TABLE PK VALUE
                array( // CAPTIONS
                    'single_caption'    => 'Table',
                    'multiple_caption'  => 'Tables',
                    'zero_caption'      => 'No Table',
                )
            );
    }
    ...
    /*
    // returned on insert and edit
    public function _callback_field_project_option($value, $primary_key){
        // Options for detail table's column with SET type
        $set_column_option_list = array();
        // Options for detail table's column with ENUM type
        $enum_column_option_list = array();
        ...
        // Prepare the data by using defined configurations and options
        $data = $this->_one_to_many_callback_field_data(
                'project_option', // DETAIL TABLE NAME
                'id', // DETAIL PK NAME
                'project_id', // DETAIL FK NAME
                $primary_key, // CURRENT TABLE PK VALUE
                $lookup_config_list, // LOOKUP CONFIGS
        ...
        // Parse the data to the view
        return $this->load->view($this->cms_module_path().'/field_project_project_option',$data, TRUE);
    }

    // returned on view
    public function _callback_column_project_option($value, $row){
        return $this->_humanized_record_count(
                'project_option', // DETAIL TABLE NAME
                'project_id', // DETAIL FK NAME
                $row->project_id, // CURRENT TABLE PK VALUE
                array( // CAPTIONS
                    'single_caption'    => 'Project Option',
                    'multiple_caption'  => 'Project Options',
                    'zero_caption'      => 'No Project Option',
                )
            );
    }
    */
    ...
    public function _after_insert_or_update($post_array, $primary_key){
        ...
        $this->_save_one_to_many(
            ...
            $data, // DATA
            $real_column_list=array('table_id', 'name', 'caption', 'priority', 'data'), // REAL DETAIL COLUMN NAMES
            $set_column_list=array(), // SET DETAIL COLUMN NAMES
            $many_to_many_config_list=array()
        );

        // SAVE CHANGES OF project_option
        /*
        $data = json_decode($this->input->post('md_real_field_project_option_col'), TRUE);
        $this->_save_one_to_many(
            'project_option', // FIELD NAME
            'project_option', // DETAIL TABLE NAME
            'id', // DETAIL PK NAME
            'project_id', // DETAIL FK NAME
            $primary_key, // PARENT PRIMARY KEY VALUE
