...
class Navigation_Model extends CMS_Model{
    ...
    public function get_all_navigation(){
        $result = array();
        $SQL = "SELECT navigation_id, navigation_name, title FROM cms_navigation";
    ...
    public function toggle_navigation($id){
        $SQL = "SELECT active FROM cms_navigation WHERE navigation_id = $id";
        $query = $this->db->query($SQL);
        $row = $query->row();
        $active = $row->active;
    ...
    public function demote_navigation($id){
        //me
        $SQL = "SELECT navigation_id, parent_id, `index` FROM cms_navigation WHERE navigation_id=$id";
        $query = $this->db->query($SQL);
        $row = $query->row();
        $my_parent_id = $row->parent_id;
        $my_index = $row->index;
        ...
        $SQL = "SELECT navigation_id, parent_id, `index`,
            (SELECT MAX(`index`) FROM cms_navigation WHERE parent_id = bro.navigation_id) AS last_child_index 
            FROM cms_navigation bro
            WHERE 
                $haveSameParent AND
                `index`=(SELECT MAX(`index`) FROM cms_navigation WHERE `index`<$my_index AND $haveSameParent) ";
        $query = $this->db->query($SQL);
        $row = $query->row();
    ...
    public function down_navigation($id){
        //me
        $SQL = "SELECT parent_id, `index` FROM cms_navigation WHERE navigation_id=$id";
        $query = $this->db->query($SQL);
        $row = $query->row();
        $my_parent_id = $row->parent_id;
        $my_index = $row->index;
        ...
        $SQL = "SELECT navigation_id, `index`  FROM cms_navigation bro 
            WHERE $where_parent AND 
                `index` = (SELECT MIN(`index`) FROM cms_navigation WHERE `index`>$my_index AND $where_parent)";
        $query = $this->db->query($SQL);
        $row = $query->row();
        $bro_navigation_id = $row->navigation_id;
        $bro_index = $row->index;
        ...
        if(isset($bro_navigation_id)){
            //swap
            $this->db->update('cms_navigation', array("index"=>$bro_index), array("navigation_id"=>$id));
            $this->db->update('cms_navigation', array("index"=>$my_index), array("navigation_id"=>$bro_navigation_id));
        }
    ...
    public function up_navigation($id){
        ...
        $SQL = "SELECT navigation_id, `index`  FROM cms_navigation bro 
            WHERE $where_parent AND 
                `index` = (SELECT MAX(`index`) FROM cms_navigation WHERE `index`<$my_index AND $where_parent)";
        $query = $this->db->query($SQL);
        $row = $query->row();
        $bro_navigation_id = $row->navigation_id;
        $bro_index = $row->index;
